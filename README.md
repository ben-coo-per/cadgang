...
- **No button mapping on the driver route.** With 3DxWare steering, the puck's buttons do whatever 3Dconnexion Settings says (Fit and the view buttons work). cadgang does not yet publish its own commands (home view, fit, etc.) to the driver for mapping — navlib supports it via a command tree; it is on the list.
